...
keywords:
  ...
  - stable diffusion
image: images/Gemini_Generated_Image_6mn7nt6mn7nt6mn7.png
format: html
draft: false
jupyter: python3
...
> **Note on compute:** Image generation with SD 1.5 requires a GPU (CUDA or Apple Silicon MPS) for practical runtimes (~1–2 min per image). On CPU, each generation takes 20–40 minutes. If running locally without a GPU, generate the images first in Google Colab (free T4 GPU), download the [`outputs/`](https://github.com/baker-jr-john/Penn_GSE/tree/main/educ_5919_001/assignments/generation_control_strategies_beyond_texts/outputs) folder, place it alongside [this file](https://github.com/baker-jr-john/Penn_GSE/blob/main/educ_5919_001/assignments/generation_control_strategies_beyond_texts/index.qmd), then render. The caching checks (`if os.path.exists(path)`) will skip regeneration and use the saved files.
